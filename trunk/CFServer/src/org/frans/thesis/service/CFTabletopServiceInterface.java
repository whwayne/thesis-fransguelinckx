 ...
 */
/**
 * The interface that forms the communication gateway between the tabeltop and mobile devices.
 * It contains method that connected mobile devices can call.
 */
...
public interface CFTabletopServiceInterface {
	 ...
	 * @param name
	 * The name of the new device. This name must be unique, so there can be no other devices that has already been connected with the same name.
	 * @return
	 * True if the name of the new device was accepted.
	 * False if the name wasn't accepted.
	 * @throws BusException
	 */
    @BusMethod
    ...
    /**
     * A method to send a piece of a file from a mobile device to the tabletop. On the tabletop the piece is joined with the rest of the file.
     * @param path
     * The orinal path of the file in the filesystem of the mobile device.
     * @param name
     * The name of the client that sends the file.
     * @param buf
     * The array of bytes that is being sent.
     * @param lastPiece
     * A boolean that indicates whether its the last piece. True indicates it is and vice versa.
     * @return
     * True if the transaction succeeded.
     * @throws BusException
     * @pre
     * Pieces of files have to be sent in the correct order, as they are reassembled consecutively.
     */
    @BusMethod
    public boolean receivePieceOfFile(String path, String name, byte[] buf, boolean lastPiece) throws BusException;
    
    /**
     * A method to indicate a mobile device is closing its connection to the tabletop.
     * @param name
     ...
     * @param name
     * The name of the device of which the status has to be set to IDLE.
     * @return
     * True if the transaction succeeded and vice versa.
     * @throws BusException
     */
    @BusMethod
    ...
    /**
     * Returns the next piece of a file that is being sent to a client with a given name.
     * @param clientName
     * The name of the client that is requesting the next piece of the file.
     * @return
     * The next piece of the file as an array of bytes.
     * Returns an array with length 1 if all data has been sent.
     */
    @BusMethod
